...
impl GeneHandler {
    pub fn get_alive_gene(&self, index: usize) -> [f32; GENE_SIZE] {
        self.gene_container[index]
    }
    pub fn initialize(&mut self) {
        for _ in 0..self.gene_count {
            let mut genes = [0f32; GENE_SIZE];
            for gene in genes.iter_mut().take(GENE_SIZE) {
                *gene = self.rng.gen_range(-16.0..16.0);
            }
    ...
    pub fn get_dna(&mut self) -> Dna {
        let mut index = 0;
        if let Some((found_index, _)) = self
            .fitness
            .iter()
            .enumerate()
    ...
    fn pick_gene_by_fitness(&mut self, accumulated_array: &[f32]) -> usize {
        let max = accumulated_array.last().unwrap();
        let rnd = self.rng.gen_range(0.0..*max);
        let mut index = 0;
        for (i, value) in accumulated_array.iter().enumerate() {
            if *value > rnd {
                index = i;
                break;
            }
    ...
    pub fn process_generation(&mut self) {
        ...
        let mut indexed_fitness: Vec<(usize, f32)> =
            self.fitness.iter().cloned().enumerate().collect();
        ...
        for index in sorted_indices.iter().take(survive_count) {
            next_generation_genes.push(self.gene_container[*index]);
        }
    ...
    fn evolve(&mut self, gene_index: usize, probability: f32) -> [f32; GENE_SIZE] {
        let mut gene = self.gene_container[gene_index];
    ...
    fn make_child(
        ...
        for (i, value) in gene.iter_mut().enumerate().take(GENE_SIZE) {
            let change: f32 = self.rng.gen_range(0.0..1.0);
            if change < probability {
                *value = self.rng.gen_range(-16.0..16.0);
            } else {
                let distrib = 1.0f32 + self.rng.gen_range(0.0f32..probability);
                *value = if i < cross_point {
