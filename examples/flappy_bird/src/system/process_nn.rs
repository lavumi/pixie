...
fn process_layer(input_array: Vec<f32>, genes: Vec<&f32>, neuron_count: usize) -> Vec<f32> {
    ...
    if gene_unit_size * neuron_count != genes.len() {
        panic!(
            "process neural network error!! input and genes count not matched {} , {}, {}",
            gene_unit_size,
            neuron_count,
            genes.len()
        )
...
pub fn process_nn(world: &mut World, resources: &mut ResourceContainer) {
    ...
    for (_entity, (player, p_tr, gene)) in world.query_mut::<(&mut Player, &Transform, &Dna)>() {
        ...
        let layer_2_gene = gene
            .genes
            .iter()
            .skip(layer_1_gene_size)
            .take(layer_2_gene_size)
            .collect();
        let layer_2 = process_layer(layer_1, layer_2_gene, hidden_layer_2_size);

        // Output layer
        let output_layer_gene_size = hidden_layer_2_size + 1;
        let output_layer_gene = gene
            .genes
            .iter()
            .skip(layer_1_gene_size + layer_2_gene_size)
            .take(output_layer_gene_size)
            .collect();
...
mod tests {
    ...
    fn test_process_layer() {
        ...
        let genes = vec![
            0.4, 0.9, 0.01, 0.3, 0.2, 0.01, 0.5, 0.1, -0.03, 0.3, 0.1, 0.07, 0.9, -0.3, 0.02, 0.4,
            0.9, 0.01, 0.3, 0.2, 0.01, 0.5, 0.1, -0.03, 0.3, 0.1, 0.07, 0.9, -0.3, 0.02, 0.4, 0.9,
            0.01, 0.3, 0.2, 0.01, 0.5, 0.1, -0.03, 0.3, 0.1, 0.07, 0.9, -0.3, 0.02,
        ];
